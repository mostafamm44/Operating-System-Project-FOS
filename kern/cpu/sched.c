...
void sched_init_PRIRR(uint8 numOfPriorities, uint8 quantum, uint32 starvThresh)
{
	...
	ProcessQueues.env_ready_queues =
	        (struct Env_Queue*)kmalloc(numOfPriorities * sizeof(struct Env_Queue));
...
struct Env* fos_scheduler_PRIRR()
{
	...
	/****************************************************************************************/
	//TODO: [PROJECT'24.MS3 - #08] [3] PRIORITY RR Scheduler - fos_scheduler_PRIR

		// we shouldn't update priority because it is already updated fos_scheduler()
	struct Env *next_env = NULL;
	struct Env *cur_env = get_cpu_proc();
...
for (int priority = 0; priority < num_of_ready_queues; priority++) {
		env = ProcessQueues.env_ready_queues[priority].lh_first;
		while (env != NULL) {
		   next_env = env->prev_next_info.le_next;
		    uint32 tick = ticks - (env->env_runs * quantums[env->priority]);
		     if (tick > starvation_threshold) {
	if (env->priority >= PRIORITY_LOW) {
		                env->priority--;
		                 acquire_spinlock(&ProcessQueues.qlock);
		                   LIST_REMOVE(&(ProcessQueues.env_ready_queues[priority]), env);
