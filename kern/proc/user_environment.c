...
void env_init(void)
{
	...
	for(; iEnv >= 0; iEnv--)
	{
		...
		envs[iEnv].env_id = 0;
		//cprintf("envs-->%x[100 in user_environment]\n",envs[iEnv]);
...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
    ...
    void* stackbottom = kmalloc(KERNEL_STACK_SIZE);
    if (stackbottom == NULL)
    {
   ...
   void * guard = stackbottom;
    uint32* page_table = NULL;

    if (get_page_table(ptr_user_page_directory,(uint32) guard, &page_table) != TABLE_IN_MEMORY)
    {
        panic("sssssssssssssssssssssss");
    }
   // page_table[PTX(guard)] = 0;
    pt_set_page_permissions(ptr_user_page_directory,(uint32)guard,0,(1<<10));
    return stackbottom;
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
	...
	e->kstart=daStart ;//ROUNDDOWN(daStart,PAGE_SIZE);
	e->khardlimit=daLimit;//ROUNDUP(daLimit,PAGE_SIZE);
	e->kbrk=e->kstart;
	//cprintf("we are in user heap init!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!?\n");
	//if(e->kbrk>daLimit)return;
