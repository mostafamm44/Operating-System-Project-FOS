...
void test_realloc_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	cprintf("5: Test reallocating to a size greater than available space [NO PREV][NO NEXT]( ALLOCATE).\n\n");
	is_correct = 1;

	{
	    blockIndex = 0; // Use an existing block
	    old_size = allocSizes[blockIndex] - sizeOfMetaData;
	    new_size = old_size + 1; // New size exceeds the current allocated space
	    ...
	    if (va == NULL) {
	        is_correct = 0;
	        cprintf("test_realloc_block_FF #5.1: Expected a valid address for a new allocation, but got NULL.\n");
	    } else {
	        // Verify that the new block is indeed a new allocation and not the same old address
	        if (va == startVAs[blockIndex]) {
	            is_correct = 0;
	            cprintf("test_realloc_block_FF #5.1: Expected a new address after reallocating, but got the same old address.\n");
	        }
	        // Additionally, verify the size of the new block
	        if (get_block_size(va) < new_size + sizeOfMetaData) {
	            is_correct = 0;
	            cprintf("test_realloc_block_FF #5.1: New block size is less than expected size.\n");
	        }
	    }
	}

	if (is_correct) {
	    eval += 10; // Increment evaluation score if test passed
	}



	cprintf("[TOTAL] test realloc_block with FIRST FIT completed. Evaluation = %d%\n", eval);
...
void test_realloc_block_FF_COMPLETE()
{
	...
	return;
#endif
	cprintf("\n test realloc_block with FIRST FIT ROUND 2 START FROM HERE \n");
	int initAllocatedSpace = 3*Mega;
		initialize_dynamic_allocator(KERNEL_HEAP_START, initAllocatedSpace);
		cprintf("\n test reallocate with decrased size and splite (merge with next)\n");
		struct BlockElement* tstblock =alloc_block_FF(1000);
		uint32 count;
		count =LIST_SIZE(&freeBlocksList);
		realloc_block_FF(tstblock,500);
		 count =LIST_SIZE(&freeBlocksList);
		if (count ==1){
			cprintf("\nDONEtest reallocate 1 correct \n");
			realloc_block_FF(tstblock,490);
					if (498==get_block_size(tstblock)){
						cprintf("\n\nDONE test reallocate 2 correct \n");
						 count =LIST_SIZE(&freeBlocksList);
										if (count ==1){
											cprintf("\nDONE test reallocate 3 correct \n");

											 struct BlockElement* tst2block=alloc_block_FF(600);
											 struct BlockElement*tsts=realloc_block_FF(tstblock,600);
											if (tstblock!=tsts){

												cprintf("\nDONE test reallocate 4\n");

												//print_blocks_list(freeBlocksList);

												if (tsts==realloc_block_FF(tsts,800)){


													cprintf("\nDONE FINAL TEST:)\n");
												//print_blocks_list(freeBlocksList);
												}
												else cprintf("WRONG on final test\n");
