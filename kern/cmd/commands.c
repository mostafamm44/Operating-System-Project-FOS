...
struct Command commands[] =
{
		...
		{ "schedPRIRR", "reads block of bytes from specific location in given environment" ,command_schedPRIRR, 3},
		//**************************************//
...
int command_schedPRIRR(int number_of_arguments, char **arguments)
{
    ...
    {
        cprintf("Usage: schedPRIRR <numOfPriorities> <quantum> <starvThresh>\n");
        return -1;
    }
    uint8 numOfPriorities = (uint8)strtol(arguments[1], NULL, 10);
    uint8 quantum = (uint8)strtol(arguments[2], NULL, 10);
    uint32 starvThresh = (uint32)strtol(arguments[3], NULL, 10);
