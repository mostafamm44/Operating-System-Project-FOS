...
uint32 sys_mark_page(uint32 virtual_address){
	...
	return mark_page(cur_env ,virtual_address);
}
...
void sys_removegive_queue(struct Env_Queue* queue)
{
	acquire_spinlock(&ProcessQueues.qlock);
	 ...
	 remove_from_queue(queue, envItem);
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_env_set_priority :
			...
			return 0;
			break;
	case SYS_removegive_queue :
		sys_removegive_queue((struct Env_Queue*) a1);
		return 0;
		break;
	...
	case SYS_init_queue :
		...
		break;
	case SYS_push_queue :
	 sys_push_queue((struct Env_Queue*) a1,(int*) a2);
	 return 0 ;
break;
	case SYS_sbrk :
