...
void *alloc_block_FF(uint32 size)
{
	...
	//==================================================================================
	{
		...
		if (!is_initialized)
		{
			//cprintf("alloc block_ff_size %d [179]\n",size);
			uint32 required_size = size + 2*sizeof(int) /*header & footer*/ + 2*sizeof(int) /*da begin & end*/ ;
			uint32 da_start = (uint32)sbrk(ROUNDUP(required_size, PAGE_SIZE)/PAGE_SIZE);

			//cprintf("[we in alloc_block_fF]intitlize start with %x\n",da_start);
			uint32 da_break = (uint32)sbrk(0);
			//cprintf(" [we in alloc_block_fF]intitlize  brk with %d ",da_break);
    ...
    uint32 num=ROUNDUP(size,4*1024)/(4*1024);
      void* brk = sbrk(num);
         ...
         struct BlockElement*oldend=(struct BlockElement*)((char*)(sbrk(0)-sizeof(int)));
          ...
          // struct BlockElement* addr=	alloc_block_FF(size);
