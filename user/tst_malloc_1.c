...
void _main(void)
{
	...
	void* ptr_allocations[20] = {0};
	{
		...
		//2 MB
		{
			...
			//cprintf("bong\n");
			if ((uint32) ptr_allocations[0] != (pagealloc_start)) {is_correct = 0; cprintf("1 Wrong start address for the allocated space... found--%x ex---%x \n",ptr_allocations[0] ,pagealloc_start);}
			...
			//cprintf("here6\n");
			//cprintf(" bytarr--%x",(uint32*)byteArr);
			byteArr[0] = minByte ;
			//cprintf("here7\n");
		...
		//2 MB
		{
			...
			ptr_allocations[1] = malloc(2*Mega-kilo);
				//cprintf("after 2megseconde\n");
			if ((uint32) ptr_allocations[1] != (pagealloc_start + 2*Mega)) { is_correct = 0; cprintf("2 Wrong start address for the allocated space...ex---%x---found--%x \n",(pagealloc_start + 2*Mega),ptr_allocations[1])	;  }
